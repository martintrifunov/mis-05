...
class TermShortestPath extends StatefulWidget {
  final Exam exam;
  const TermShortestPath(this.exam);

  @override
  _TermShortestPathState createState() => _TermShortestPathState(this.exam);
}

class _TermShortestPathState extends State<TermShortestPath> {
  Exam exam;
  bool flag = true;

  _TermShortestPathState(this.exam);
  ...
  _getCurrentLocationAndCalculatePath() async {
    startAddressFocusNode.unfocus();
    desrinationAddressFocusNode.unfocus();
    setState(() {
      _destinationAddress = this.exam.location!;
  ...
  Future<bool> _calculateDistance() async {
    try {
      ...
      if (flag) {
        ...
        this.flag = false;
        destinationLatitude = double.parse(this.exam.lat!);
        destinationLongitude = double.parse(this.exam.long!);
      } else {
  ...
  _createPolylines(
    ...
    PolylineResult result = await polylinePoints.getRouteBetweenCoordinates(
      'AIzaSyC96o3Rzef6rAkLEivdVehFmq9fNOmuBfU',
  ...
  void initState() {
    ...
    _getCurrentLocationAndCalculatePath();
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Scaffold(
        key: _scaffoldKey,
        appBar: AppBar(
          title: Text('Terms shortest path'),
        ),
        body: Stack(
          children: <Widget>[
            GoogleMap(
